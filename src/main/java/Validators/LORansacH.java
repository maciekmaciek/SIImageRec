...
/**
 * Created by Maciej Wola雟ki
 * maciekwski@gmail.com
 * on 2015-06-01.
 */
public class LORansacH extends Ransac {

    public LORansacH(ArrayList<Pair<VectorPoint, VectorPoint>> generatedPairs) {
        super(generatedPairs);
    }
    private final int LO_REPS = 100;
    private ArrayList<Pair<VectorPoint, VectorPoint>> localValidPairs = new ArrayList<>();
    ...
    public Matrix findTransform() {
        ...
        for(int counter = 0; counter <= 100000; counter++){         //RANSAC
            ...
            int range = neighbourhood.get(i).size();                //zaw篹
            int rangeLimit = range*range;                //瞠by nie blokowa這
            int numoftries = 0;                                 //瞠by nie blokowa這
            if(range >= 3){
                int j;
                do {
                ...
                try {
                    if (!(p1 == null || p2 == null || p3 == null || p4 == null)) {
                        ...
                        if (score > bestScore) {
                            ...
                            //LO STEP
                            if(localOpt(bestScore) > bestScore){
                                counter+=LO_REPS;
                                bestModel = localModel;
                                validPairs.clear();
                                validPairs.addAll(localValidPairs);
                            }
                        }
                    }
    ...
    private int localOpt(int bestScore){

        int size = validPairs.size();
        Random random = new Random();
        ...
        Pair<VectorPoint,VectorPoint> p3 = null;
        Pair<VectorPoint,VectorPoint> p4= null;
        ArrayList<Pair<VectorPoint, VectorPoint>> tempLocalPairs = new ArrayList<>();

        for(int count = 0; count < LO_REPS; count++){
            tempLocalPairs.clear();
            int score = 0;
            Matrix model;

            int i = random.nextInt(size);                           //losuj
            p1 = validPairs.get(i);
            int range = validPairs.size();                //zaw篹
            int rangeLimit = range*range;                //瞠by nie blokowa這
            int numoftries = 0;                                 //瞠by nie blokowa這
            if(range >= 3){
                int j;
                ...
                do {
                    ...
                    k = random.nextInt(range);
                    p3 = validPairs.get(k);
                } while ((k == i || k == j || !PairValidator.validateNeighbourNear(p2, p3)) && numoftries <= rangeLimit);
                if(numoftries < rangeLimit){
                    numoftries = 0;
                    int l;
                    do {
                        ...
                        p4 = validPairs.get(l);
                            ...
                            !PairValidator.validateNeighbourNear(p2, p4) ||
                            !PairValidator.validateNeighbourNear(p3, p4)) &&
                            numoftries <= rangeLimit);
                }
                ...
                try {
                    if (!(p1 == null || p2 == null || p3 == null || p4 == null)) {
                        ...
                        if (score > bestScore) {
                            bestScore = score;
                            localModel = model;
                            localValidPairs.clear();
                            localValidPairs.addAll(tempLocalPairs);
                        }
                    }
                } catch (RuntimeException re) {
                    re.printStackTrace();
